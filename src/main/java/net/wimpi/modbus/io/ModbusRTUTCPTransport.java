	...
	public synchronized void writeMessage(ModbusMessage msg) throws ModbusIOException
	{
		try
		{
			...
			synchronized (this.outputBuffer)
			{
				...
				// debug
				// if (Modbus.debug)
				logger.debug("Sent: " + ModbusUtil.toHex(rawBuffer, 0, bufferLength));
	...
	public synchronized ModbusResponse readResponse() throws ModbusIOException
	{
		...
		try
		{
			...
			synchronized (inputBuffer)
			{
				...
				// debug
				logger.debug(ModbusRTUTCPTransport.logId + "Read packet with progressive id: " + packetId);
				
				// read the function code
				int functionCode = inputBuffer.readUnsignedByte();
				
				// debug
				logger.debug(" uid: " + packetId + ", function code: " + functionCode);
				...
				while ((this.inputStream.available() < (packetLength - 3)) && (!this.isTimedOut))
				{
					...
					{
						// do nothing
						logger.error("Sleep interrupted while waiting for response body...\n"+ie);
					}
				}
				...
				// debug
				logger.debug(" bytes: " + ModbusUtil.toHex(inBuffer, 0, packetLength) + ", desired length: "
						+ packetLength);
		...
		{
			// debug
			logger.error(ModbusRTUTCPTransport.logId + "Error while reading from socket: " + e);
			...
			{
				// debug
				logger.error(ModbusRTUTCPTransport.logId + "Error while emptying input buffer from socket: " + e);
			}
